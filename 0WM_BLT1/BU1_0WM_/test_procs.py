        ...
        for c in args['Cmds']:
            ...
            if c.get('Results'):
                ...
                for r in c['Results']:
                    ...
                    if (len(result_list[r['name']]) > 0) and r.has_key("subtype"):
                        if r.get('subtype') == 'int':
                            if is_number(result_list[r['name']]):
                                result_list[r['name']] = int(float(result_list[r['name']]))
                        elif r.get('subtype') == 'float':
                            if is_number(result_list[r['name']]):
                                result_list[r['name']] = float(result_list[r['name']])
                        else:
                            result_list[r['name']] = result_list[r['name']][r['subtype'][0] : r['subtype'][1]]

                    if r.has_key('unit'):
                        result_list[r['name']] = str(result_list[r['name']]) + r.get('unit')
...
# ======== Function begin ========
def DeviceReady(test, args):
    return api.machine_wait_for_boot(DISPLAY_ENABLE=args['Display'],TIMEOUT=args['Timeout'])

def DeviceReboot(test, args):
    return api.machine_reboot(REBOOT_CMD=args['Cmd'],DISPLAY_ENABLE=args['Display'],TIMEOUT=args['Timeout'])
...
def RunMFMStart(test, args):
    ...
    if args.has_key("Run"):
        ...
        if args.has_key("Retry"):
            retry = args['Retry']
            Logger.record({'Retry': retry})
        if args.has_key("Search"):
            search_str = args['Search']
            Logger.record({'Search': search_str})
    else:
        result = "RunMFMStart: ERROR"
    result = api.run_manufacturing_mode( cmd, PARA=para, SLEEP=sleep, RETRY=retry, SERACH=search_str )
    Logger.record(result)
    return result

def ShowPassFailGui(test, args):
    test_item = 'default'
...
def WriteMacAddress(test, args):
    ...
    if args.has_key("chip"):
        chip = args['chip']
        Logger.record({'chip': chip})

    result = api.WriteMacAddress(chip)
