...
#    env.print_warn( scan_dict )
    ...
    for find_ap in AccessPoint:
        if find_ap in scan_dict:
            scan_dict[find_ap] = scan_dict[find_ap][:-3]
...
def machine_wait_for_boot(DISPLAY_ENABLE=True, DISPLAY_TIME=1):
    env.print_warn("\n>> Waiting for Boot <<")
    results = ''
    found = False
    initial = time.time()
...
def machine_reboot(REBOOT_CMD='reboot',DISPLAY_ENABLE=True, DISPLAY_TIME=1):
...
#                            return machine_reboot(DISPLAY_ENABLE=False, DISPLAY_ENABLE=False, DISPLAY_TIME=1)
                            return True
                        else:
                            return False
                    else:
...
#                or results.find("command not found")!=-1 or results.find("command failed")!=-1 or results.find("Unknown Command")!=-1
                or results.lower().find("error")!=-1 or results.lower().find("command")!=-1
